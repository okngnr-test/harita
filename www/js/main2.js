$(document).ready(function(){
	...
	var retrievedObject = localStorage.getItem('mapJson1117');
	...
	function initMap()
	{
		...
		if(mapJSON == null)
		{
			$.getJSON('map.json', function(data){
				...
				{
					localStorage.setItem('mapJson1117', JSON.stringify(data));
	...
	function onCompassSuccess(heading) {
		...
		$('#map-wrapper').css('transform', srotate);
    }
	...
	function onPositionSuccess(position)
	{
		...
		img.setAttributeNS('http://www.w3.org/1999/xlink','href','img/direction_arrow.png');
		img.setAttributeNS(null, 'x', 100);
		img.setAttributeNS(null, 'y', 100);
		
		$('#map-wrapper').find('svg').append(img);
		
		$('#map-wrapper').html($('#map-wrapper').html());
		
		console.log($('#map-wrapper').find('svg').find('image').attr('x'));
	}
	...
	function initMapDrawer()
	{
		if(typeof(Worker)!=="undefined")
		{
			...
			mapDrawer.onmessage = function(event){
				...
				$('#map-container').append('<div id="map-wrapper" style="margin:0 auto;width:'+defaultMapWidth * Math.pow(2, zoomLevel-1)+'px; height:'+defaultMapHeight * Math.pow(2, zoomLevel-1)+'px;">'+event.data+'</div>');
